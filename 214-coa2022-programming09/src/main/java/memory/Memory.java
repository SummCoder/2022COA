...
public class Memory {
    ...
    public void seg_load(int segIndex) {
        // TODO
        if(Memory.SEGMENT && !Memory.PAGE){
            write("00000000000000000000000000000000", Integer.parseInt((Transformer.binaryToInt(0 + "11111111111111111111"))), disk.read("00000000000000000000000000000000", Integer.parseInt(Transformer.binaryToInt(0 + "11111111111111111111"))));
        }else {
            getSegDescriptor(segIndex).granularity = true;
        }
        getSegDescriptor(segIndex).validBit = true;
        for (int i = 0; i < 32; i++) {
            getSegDescriptor(segIndex).base[i] = '0';
        }
        for (int i = 0; i < 20; i++) {
            getSegDescriptor(segIndex).limit[i] = '1';
        }
    }
    ...
    public void page_load(int vPageNo) {
        // TODO
        String pAddr = Transformer.intToBinary(String.valueOf(vPageNo)).substring(12) + "000000000000";
        for (int i = 0; i < MEM_SIZE_B / PAGE_SIZE_B; i++) {
            if(!pageValid[i]){
                getPageItem(vPageNo).isInMem = true;//已经加载到内存中，置为true
                String pageF = Transformer.intToBinary(String.valueOf(i)).substring(12) + "000000000000";
                for (int j = 0; j < 20; j++) {
                    getPageItem(vPageNo).pageFrame[j] = pageF.charAt(j);
                }
                write(pageF, PAGE_SIZE_B, disk.read(pAddr, PAGE_SIZE_B));
                pageValid[i] = true;
                break;
            }
        }
    ...
    private static class PageItem {
        ...
        private char[] pageFrame = new char[20];   // 物理页框号
