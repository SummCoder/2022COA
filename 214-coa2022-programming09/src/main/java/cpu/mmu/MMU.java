...
public class MMU {
        ...
        if (!Memory.SEGMENT) {
            ...
            if (!Memory.PAGE) {
                ...
                for (int i = startvPageNo; i <= endvPageNo; i++) {
                    // TODO: add tlb here
                    if(!tlb.isValidPage(i)){//即不在tlb，也不在主存中
                        memory.page_load(i);
                        tlb.write(i);
                    }
    ...
    private String toRealLinearAddr(String logicAddr) {
        // TODO
        String baseAddr = "000000000000" + logicAddr.substring(0, 16) + "0000";
        String offset = "0000000000000000" + logicAddr.substring(32);
        String linearAddr = new ALU().add(new DataType(baseAddr), new DataType(offset)).toString();
        return linearAddr;
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        // TODO
        int segIndex = getSegIndex(logicAddr);
        String baseAddr = new String(Memory.getMemory().getBaseOfSegDes(segIndex));
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        // TODO: add tlb here
        int startvPageNo = Integer.parseInt(Transformer.binaryToInt(linearAddr.substring(0, 20)));   // 高20位表示虚拟页号
        String offset = "00000000000000000000"+ linearAddr.substring(20, 32);//页内偏移
        String phyAddr;
        if(TLB.isAvailable){
            if(tlb.getFrameOfPage(startvPageNo) != null){
                phyAddr = new ALU().add(new DataType(new String(TLB.getTLB().getFrameOfPage(startvPageNo)) + "000000000000"), new DataType(offset)).toString();
            }else{
                tlb.write(startvPageNo);
                phyAddr = new ALU().add(new DataType(new String(Memory.getMemory().getFrameOfPage(startvPageNo)) + "000000000000"), new DataType(offset)).toString();
            }
        }
        else{
            tlb.write(startvPageNo);
