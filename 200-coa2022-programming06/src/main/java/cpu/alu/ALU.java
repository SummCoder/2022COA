...
public class ALU {
    ...
    public String add(String src, String dest) {
        return add(new DataType(src), new DataType(dest)).toString();
    }
    public String sub(String src, String dest){
        return sub(new DataType(src), new DataType(dest)).toString();
    }
    public String mul(String src, String dest){
        ...
        for (int i = 0; i < length; i++) {
            ...
            product = product.charAt(0) + product.substring(0, product.length() - 1);  //算数右移
            Y1 = Y2;  //更新两个Y
            Y2 = product.charAt(2 * length - 1) - '0';
        }
        return product.substring(10);
    }
    public String div(String src, String dest){
        return div(new DataType(src), new DataType(dest)).toString();
